...
const mainPilotOut = vscode.window.createOutputChannel("mainPilot");
            ...
            .then((completion) => {
                ...
                if (vscode.window.activeTextEditor) {
                    vscode.window.activeTextEditor.edit(editBuilder => {
                        ...
                        editBuilder.replace(new vscode.Range(selection.start, selection.end), completion);
                    });
            }});
	});
}
...
export async function activate(context: vscode.ExtensionContext) {
	...
	const predict = vscode.commands.registerCommand("mainPilot.predict", async function() {
		appendCurrentSelection(context);
	});
	context.subscriptions.push(predict);

    const setLevelAndPredict = vscode.commands.registerCommand("mainPilot.setLevelAndPredict", async function() {
